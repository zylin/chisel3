...
sealed class Vec[T <: Data] private[chisel3] (gen: => T, val length: Int) extends Aggregate with VecLike[T] {
  ...
  def do_reduceTree(
    ...
    while (curLayer.length > 1) {
      curLayer = VecInit(curLayer.grouped(2).map(x => if (x.length == 1) layerOp(x(0)) else redOp(x(0), x(1))).toSeq)
    }
    curLayer(0)
  }
...
abstract class Bundle(implicit compileOptions: CompileOptions) extends Record {
  ...
  protected def _cloneTypeImpl: Bundle = {
    assert(
      ...
      "reflective autoclonetype is disallowed, this should only happen in testing"
    )
    Builder.deprecated(
            ...
            case (_: NoSuchFieldException | _: IllegalAccessException) =>
              ...
              allOuterCandidates.filter(canAssignOuterClass(_)) match {
                ...
                case Nil => // TODO: replace with fatal autoClonetypeError once compatibility period is dropped
                  outerClassError = Some(
                    s"Unable to determine instance of outer class $outerClass," +
      ...
      val clone = (argTypes, outerClassInstance) match {
        ...
        case (argType :: Nil, Some((_, outerInstance))) =>
          if (outerInstance == null) {
            Builder.deprecated(
              s"chisel3.1 autoclonetype failed, falling back to 3.0 behavior using null as the outer instance." +
    ...
    val classSymbol = classSymbolOption.getOrElse(
      autoClonetypeError(
        s"scala reflection failed." +
    ...
    if (ctors.size != 1) {
      autoClonetypeError(
        s"found multiple constructors ($ctors)." +
    ...
    if (
      ctorParams.size == 1 && outerClassInstance.isDefined &&
    ...
    if (!paramsDiff.isEmpty) {
      autoClonetypeError(
        s"constructor has parameters (${paramsDiff.toList.sorted.mkString(", ")}) that are not both immutable and accessible." +
    ...
    val ctorParamsNameVals = ctorParamsNames.map { paramName =>
    ...
    if (boundDataParamNames.nonEmpty) {
      autoClonetypeError(
        s"constructor parameters (${boundDataParamNames.sorted.mkString(", ")}) have values that are hardware types, which is likely to cause subtle errors." +
